import React, { Component } from "react";
import { connect } from 'react-redux';
import { NavLink, Link, withRouter } from "react-router-dom";
import { logout, changeProfilePic } from "../Auth/authAction";
// import socketIOClient from "socket.io-client";
// const date= new Date()
// const socket = socketIOClient("http://127.0.0.1:8000");

import "./header.css";
const user = "data:image/png;base64,"

class Header extends Component {
    constructor(props) {
        super(props);
        if (props.user.picture == undefined) {
            var picture = user
        }
        else {
            var picture = props.user.picture
        }
        this.state = {
            picture: picture
        };
    }
    handleLogout(event) {
        event.preventDefault();
        this.props.dispatch(logout(this.props));
    }
    handleClick(e) {
        this.refs.fileUploader.click();
    }
    onChangeFile(e) {
        var file = e.target.files[0]
        let reader = new FileReader()
        reader.readAsDataURL(file)
        reader.onload = () => {
            this.setState({
                picture: reader.result,
            })
            this.props.dispatch(changeProfilePic(reader.result, this.props.token, this.props))
        }
    }
    render() {
        var username = " "
        var profilePic = ""
        if (this.props.user) {
            username = this.props.user.name
            profilePic = this.state.picture
        }
        return (
            <header className="masthead mb-auto">
                <div className="inner">
                    <div className=" masthead-brand navbar-brand"  >
                        <img src={profilePic} className="profilePicture" alt="" />
                        <span className="badge badge-add btn" onClick={() => this.handleClick()}>
                            <i className="fa fa-plus " style={{ fontSize: "20px" }}></i>
                        </span>
                        <input type="file" id="file" ref="fileUploader" style={{ display: "none" }} onChange={(e) => this.onChangeFile(e)} />
                        <h4 style={{ display: "inline", paddingLeft: "15px" }}>{username}</h4>
                    </div>
                    <nav className="nav nav-masthead justify-content-center">
                        <div className="btnPadding">
                            <button to="/" className="btn btn-notification">
                                <i className="fa fa-bell-o " style={{ fontSize: "30px" }}></i>
                            </button>

                            <span className="badge badge-notify">3</span>

                        </div>
                        <div className="btnPadding">
                            <button className="btn btn-changePass" onClick={() => this.props.history.push("/change-password")}>Change Password</button>
                        </div>
                        <div className="btnPadding">
                            <button className="btn btn-logout" onClick={this.handleLogout.bind(this)}>Logout</button>
                        </div>
                    </nav>

                </div>
            </header>

        );
    }
}

const mapStateToProps = (state) => {
    return {
        token: state.auth.token,
        user: state.auth.user
    };
};

export default withRouter(connect(mapStateToProps)(Header));
